...
class ModaccordeonckHelper {
		...
		if ($items) {
			...
			foreach ($items as $i => $item) {
				...
				if ($item->insertmodule AND $item->module) {
					$item->content = '<div class="accordeonckmod">' . ModaccordeonckHelper::GenModuleById($item->module, $params, $modulesList, $item->params->get('accordeonckparams_forcemoduletitle', 0)) . '<div style="clear:both;"></div></div>';
	...
	static function GenModuleById($title, &$params, &$modulesList, $forcetitle = false) {
		$attribs['style'] = 'xhtml';
		...
		if (JModuleHelper::isEnabled($modname)) {
			...
			if ($module) {
				$rendertitle = $forcetitle ? '<h3>' . $modtitle . '</h3>' : '';
				return $rendertitle . JModuleHelper::renderModule($module, $attribs);
			}
		}
